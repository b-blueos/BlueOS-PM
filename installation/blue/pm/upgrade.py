        ...
        if os.path.exists(sys.argv[1]):
            if os.path.isfile(sys.argv[1]):
                if sys.argv[1].endswith('.tar.xz'):
                    update_found = True
                    print(f'Moving {sys.argv[1]} to /blue/update/update.tar.xz to prepare for update...')
                    os.system(f"mv {sys.argv[1]} /blue/update/update.tar.xz")
                else:
...
if update_found == True:
    ...
    os.system(f"mkdir /tmp/blue_update && tar -C /tmp/blue_update/ -xf /blue/update/update.tar.xz && chmod +x /tmp/blue_update/index.sh")
    print(f'Starting the update...')
    os.system(f"bash /tmp/blue_update/index.sh")
    print(f'Creating new manifest...')
    os.system(f"cp -av /tmp/blue_update/manifest.json /blue/pm/manifest.json")
    print(f'Cleaning up...')
    os.system(f"rm -rf /blue/update/* /tmp/blue_update/")
